        ...
        private async Task<dynamic> GetPageData()
        {
            try
            {
                ...
                if (HttpContext != null && HttpContext.Session != null)
                {
                    ...
                    string queryString = "$filter=deleted eq false&$orderby=Rank";
                    var response = await _bosAuthClient.GetFilteredRolesAsync<Role>(queryString); //Making a BOS API call to get a list of All the roles in the application
                    if (response.IsSuccessStatusCode)
                    {
