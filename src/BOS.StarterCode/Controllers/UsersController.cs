...
namespace BOS.StarterCode.Controllers
{
    ...
    public class UsersController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            if (model != null)
            {
                var prop = (IDictionary<String, Object>)model;
                if (prop["Status"] != null && Convert.ToString(prop["Status"]).Contains("Token Expired"))
                {
                    return RedirectToAction("SignOut", "Auth");
                }
            }
            return View(model); //Returns to the View with data that is required to render the page
        }
                ...
                if (data != null) //Confirm non-null input data
                {
                    ...
                    if (userObj != null) //Checking for a non-null userObj
                    {
                        if (userObj.Username != null && userObj.Email != null && password != null) //Non-null values
                        {
                            ...
                            {
                                var result = await _bosAuthClient.AddNewUserAsync<BOSUser>(userObj.Username, userObj.Email, password, userObj.FirstName, userObj.LastName); //Making a BOS API call to add a new user record
                                        ...
                                        if (roleResponse != null && roleResponse.IsSuccessStatusCode)
                                        {
                                            ...
                                            if (isEmailToSend)
                                            {
                                                ...
                                                if (slugResponse != null && slugResponse.IsSuccessStatusCode)
                                                {
                                                    ...
                                                    emailObj.Substitutions.Add(new Substitution { Key = "applicationUrl", Value = baseUrl});
                                                    ...
                                                    emailObj.Substitutions.Add(new Substitution { Key = "password", Value = password });
        ...
        private async Task<dynamic> GetPageData()
        {
            try
            {
                ...
                if (HttpContext != null && HttpContext.Session != null)
                {
                    ...
                    {
                        //return "Token Expired, Please try again";//Token Expired
                        model.Status = "Token Expired, Please try again";
                    }
